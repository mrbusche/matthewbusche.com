...
Reveal.addEventListener("ready", function () {
	...
	test("Reveal.getSlideBackground", function () {
		...
		equal(
			...
			document.querySelector(
				".reveal .backgrounds>.slide-background:nth-child(2) .slide-background:nth-child(1)",
			),
			"v index 0 returns first vertical child",
		);
		equal(
	...
	test("background images", function () {
		...
		ok(
			Reveal.getSlideBackground(0).style.backgroundImage.indexOf(
				imageSource1,
			) !== -1,
			"data-background-image worked",
		);
		ok(
			Reveal.getSlideBackground(1, 0).style.backgroundImage.indexOf(
				imageSource2,
			) !== -1,
